...
// 3.
var filterSpecies = dogs.filter(item => item.species.includes("dog")
...
// 5. 
var dogsWithClasses = dogContestants.map((item) => {
    if (item.weight < 11){
        item.class = "red";
...
// 7.
var votes = dogs.reduce(function(totalVotes, item){
 return totalVotes + item.votes;
